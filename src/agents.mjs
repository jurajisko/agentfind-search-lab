...
const SIGNATURES = [
  ...
  // Google's documented user-triggered fetchers (docs updated 2026-08-19).
  [/google-agent/i, 'Google-Agent', 'ai', 'user_fetch'],
  [/google-gemininotebook/i, 'Google-GeminiNotebook', 'ai', 'user_fetch'],
  [/google-site-verification/i, 'Google Site Verifier', 'monitor'],
  ...
  [/oai-adsbot/i, 'OAI-AdsBot (OpenAI)', 'ai', 'mixed'],
  ...
  [/mistralai-index/i, 'MistralAI-Index', 'ai', 'ai_search'],
  [/mistralai-training/i, 'MistralAI-Training', 'ai', 'training'],
  // xAI publishes no crawler documentation; these tokens are only claimed by
  // third parties. Grok was observed without any of them (2026-09-12).
  [/grokbot|xai-bot|grok-deepsearch|xai-grok/i, 'Grok / xAI (neoficialny token)', 'ai', 'mixed'],
  ...
  [/meta-webindexer/i, 'Meta-WebIndexer', 'ai', 'ai_search'],
  [/meta-externalads/i, 'Meta-ExternalAds', 'ai', 'mixed'],
...
export const ROLES = {
  ...
  'Bingbot': { must: true, why: 'Bing, a s nim Copilot a DuckDuckGo. Copilot bez Bing indexu stranku nevidi, ani s priamym odkazom (overene 12.9.2026). Citacie v Copilote ukazuje Bing Webmaster, karta AI Performance.' },
  'OAI-SearchBot (OpenAI)': { must: true, why: 'Index pre vyhladavanie v ChatGPT. Rozhoduje, ci ta ChatGPT odcituje.' },
  'ChatGPT-User (OpenAI)': { must: true, why: 'Nacitanie nazivo pre pouzivatela ChatGPT. Podla OpenAI sa na neho robots.txt nemusi vztahovat. V nasom teste ChatGPT prisiel ako curl.' },
  'Claude-SearchBot (Anthropic)': { must: true, why: 'Index pre vyhladavanie v Claude.' },
  'Claude-User (Anthropic)': { must: true, why: 'Nacitanie nazivo pre pouzivatela Claude. Podla Anthropic robots.txt respektuje, takze jeho blokovanie Claude naozaj zastavi.' },
  ...
  'Perplexity-User': { must: true, why: 'Nacitanie nazivo pre pouzivatela Perplexity. Podla Perplexity robots.txt vacsinou ignoruje.' },
